...
export const EXAMPLE_REGISTRY: Record<string, ExampleGroup> = {
  '/components/agenthub/chat': {
    ...
    examples: [
      ...
      },
      {
        ...
        component: React.lazy(() => import('@/components/AgentHub/Chat/examples/ChatCapabilityStripBasicDemo')),
        sourcePath: () => import('@/components/AgentHub/Chat/examples/ChatCapabilityStripBasicDemo/index.tsx?raw')
      },
      {
        title: '面板头部工具',
        description: 'ChatPanelHeaderTools 缩放档位、新建会话、全屏切换（受控）',
        component: React.lazy(() => import('@/components/AgentHub/Chat/examples/ChatPanelHeaderToolsBasicDemo')),
        sourcePath: () => import('@/components/AgentHub/Chat/examples/ChatPanelHeaderToolsBasicDemo/index.tsx?raw')
      },
      {
        title: '能力条组合面板',
        description: 'Fab/Panel：headerExtra=Strip + headerActions=HeaderTools + afterMessages=ChatFollowUpSuggestions',
        component: React.lazy(() => import('@/components/AgentHub/Chat/examples/ChatPanelComposeDemo')),
        sourcePath: () => import('@/components/AgentHub/Chat/examples/ChatPanelComposeDemo/index.tsx?raw'),
        block: true
      },
      {
        title: '浮动助手 FAB',
        description: 'ChatAgentFab + ChatAgentFabLayout：引用侧栏在面板内右侧；panelFullscreen 约 4:3；headerActions 切换全屏',
    ...
    apiDoc: [
      ...
      },
      {
        ...
        rows: [
          ...
          { prop: 'headerExtra', desc: '头部下方扩展区（如能力发现条 ChatCapabilityStrip）', type: 'ReactNode' },
          { prop: 'afterMessages', desc: '消息列表内、messages 之后渲染，参与同一滚动（如 ChatFollowUpSuggestions）', type: 'ReactNode' },
      ...
      },
      {
        ...
        rows: [
          { prop: 'capabilities', desc: '能力项列表 { id, label, examples }', type: 'ReadonlyArray<ChatCapabilityItem>', required: true },
          { prop: 'expandedId', desc: '当前展开的能力 id；null 为全部收起', type: 'string | null', required: true },
          { prop: 'onExpandedChange', desc: '展开切换回调', type: '(id: string | null) => void', required: true },
          { prop: 'onSelectExample', desc: '点击样例 chip 回调', type: '(text: string) => void', required: true },
      ...
      },
      {
        ...
        rows: [
          ...
          { prop: 'onContentZoomChange', desc: '缩放变更', type: '(z: number) => void', required: true },
          { prop: 'zoomSteps', desc: '缩放档位', type: 'readonly number[]', defaultVal: '[0.2,0.35,0.5,0.7,0.85,1,1.1]' },
      ...
      },
      {
        ...
        rows: [
          ...
          { prop: 'panelFullscreen', desc: '约 4:3 拉宽拉高；引用侧栏加宽仍用 panelExpanded', type: 'boolean', defaultVal: 'false' },
      ...
      },
      {
        ...
        rows: [
          ...
          { prop: 'panelFullscreen', desc: '约 4:3 全屏（宽高随视口），仍锚定右下角 FAB', type: 'boolean' },
  ...
  '/components/agenthub/report': {
    title: 'Report 报告模板',
    description: 'AgentHub 报告布局：Meta 模板、导出工作台（api DI）、主从工作台。',
    examples: [
      {
        title: '报告模板',
    ...
    apiDoc: [
      {
        ...
        rows: [
          { prop: 'mode', desc: 'app=本系统；platform=多系统（多系统列）', type: '\'app\' | \'platform\'', defaultVal: '\'app\'' },
          { prop: 'dataSource', desc: '列表行（操作头）', type: 'AuditEventListItem[]', required: true },
          { prop: 'loadDetail', desc: '展开懒加载 steps + requestCurl', type: '(id: string) => Promise<AuditEventDetail>' },
          { prop: 'canReadTrace', desc: 'false 时展开区提示无明细权限', type: 'boolean', defaultVal: 'true' },
    ...
    apiDoc: [
      ...
      },
      {
        ...
        rows: [
          ...
          { prop: 'extra', desc: '右侧扩展区（如 InboxBell）；点击不打开用户菜单', type: 'React.ReactNode' },
  ...
  '/components/buttongroup': {
    ...
    description: '业务操作组 SSOT：薄封装 @kne/button-group。默认 unavailable→hidden（无权限/态到了不可再操作不置灰占位）；短暂 loading 用 loading/disabled。',
    examples: [
      {
        title: '1. 基础用法',
      ...
      },
      {
        ...
        component: React.lazy(() => import('@/components/ButtonGroup/examples/HideUnavailableDemo')),
        sourcePath: () => import('@/components/ButtonGroup/examples/HideUnavailableDemo/index.tsx?raw'),
        block: true
      }
    ],
    apiDoc: [
      {
        ...
        rows: [
          { prop: 'list', desc: '操作项对象数组（或函数项）。支持 unavailable：业务态不可再操作 → 默认收成 hidden', type: 'ActionListItem[]', defaultVal: '—' },
          { prop: 'hideUnavailable', desc: '默认 true：把 unavailable 合并进 hidden；false 透传（逃生）', type: 'boolean', defaultVal: 'true' },
          { prop: 'moreType', desc: '溢出「更多」形态；表格操作列常用 link（省略号）', type: '\'link\' | string', defaultVal: '—' },
          { prop: 'showLength', desc: '固定外露个数；不传则按容器宽度自动收起', type: 'number', defaultVal: '—' }
        ]
      },
      {
        componentName: 'ActionListItem / actionListItem()',
        rows: [
          { prop: 'unavailable', desc: '业务态不可再操作；hideUnavailable 时 → hidden，禁止改文案为「已认领」并 disabled 占位', type: 'boolean', defaultVal: '—' },
          { prop: 'hidden', desc: '不渲染（权限等）；与 Unavailable 任一为真即隐藏', type: 'boolean', defaultVal: '—' },
          { prop: 'actionListItem.can / available', desc: 'helper：can=权限，available=业务可操作；任一 false → hidden', type: 'boolean', defaultVal: 'true' }
        ]
      }
    ]
  },
  ...
  '/components/disposition/alertdispositionpanel': {
    ...
    description: '等级展示由项目注入 DispositionAlertLevelCatalog（key/label/tone）；core 不写死级数与文案。点色点展开「触发：」+ DispositionBar；可收起。',
    examples: [
      {
        title: '1. 可切换等级目录 + 折线',
        description: 'Radio 切换「三级制 / 预警·报警」两套 catalog；同一折线点选展开',
        component: React.lazy(() => import('@/components/Disposition/AlertDispositionPanel/examples/RevealCollapseDemo')),
        sourcePath: () => import('@/components/Disposition/AlertDispositionPanel/examples/RevealCollapseDemo/index.tsx?raw'),
        block: true
      },
      {
        title: '2. 目录驱动切换上下文',
        description: 'buildDispositionAlertContext(catalog, key) 更新触发条',
        component: React.lazy(() => import('@/components/Disposition/AlertDispositionPanel/examples/ReclickUpdateDemo')),
        sourcePath: () => import('@/components/Disposition/AlertDispositionPanel/examples/ReclickUpdateDemo/index.tsx?raw'),
        block: true
      },
      {
        title: '3. 关闭 / 空 context',
        description: 'open=false 或 context=null → 不渲染',
        component: React.lazy(() => import('@/components/Disposition/AlertDispositionPanel/examples/ClosedEmptyDemo')),
        sourcePath: () => import('@/components/Disposition/AlertDispositionPanel/examples/ClosedEmptyDemo/index.tsx?raw')
      }
    ],
    apiDoc: [
      {
      ...
      },
      {
        ...
        rows: [
          { prop: 'open', desc: '是否展开', type: 'boolean', required: true },
          { prop: 'context', desc: '触发上下文；建议 buildDispositionAlertContext(catalog, key)', type: 'DispositionAlertContext | null', required: true },
          { prop: 'children', desc: '展开区内容（通常 DispositionBar）', type: 'ReactNode', required: true },
  ...
  '/components/disposition/dispositionbar': {
    ...
    description: '布局壳：左状态徽章 + 右操作区（业务钮由调用方注入；不绑 API/EP）。点预警展开见同域 AlertDispositionPanel。',
    examples: [
      {
        title: '1. 基础用法',
        description: '默认 label「处置状态」+ string 徽章 + 操作钮',
        component: React.lazy(() => import('@/components/Disposition/DispositionBar/examples/BasicDemo')),
        sourcePath: () => import('@/components/Disposition/DispositionBar/examples/BasicDemo/index.tsx?raw'),
        block: true
      },
      {
        title: '2. 加载中藏钮',
        description: 'statusLoading=true 时不渲染 actions',
        component: React.lazy(() => import('@/components/Disposition/DispositionBar/examples/StatusLoadingDemo')),
        sourcePath: () => import('@/components/Disposition/DispositionBar/examples/StatusLoadingDemo/index.tsx?raw')
      },
      {
        title: '3. 仅状态',
        description: '无 actions（如已关闭）',
        component: React.lazy(() => import('@/components/Disposition/DispositionBar/examples/StatusOnlyDemo')),
        sourcePath: () => import('@/components/Disposition/DispositionBar/examples/StatusOnlyDemo/index.tsx?raw')
      },
      {
        title: '4. 自定义 label',
        description: 'label 覆盖默认「处置状态」',
        component: React.lazy(() => import('@/components/Disposition/DispositionBar/examples/CustomLabelDemo')),
        sourcePath: () => import('@/components/Disposition/DispositionBar/examples/CustomLabelDemo/index.tsx?raw')
      },
      {
        title: '5. 自定义徽章',
        description: 'statusBadge 传 ReactNode（如 antd Tag）',
        component: React.lazy(() => import('@/components/Disposition/DispositionBar/examples/CustomBadgeDemo')),
        sourcePath: () => import('@/components/Disposition/DispositionBar/examples/CustomBadgeDemo/index.tsx?raw')
      },
      {
        title: '6. 多操作钮（ButtonGroup）',
        description: '右侧注入 core ButtonGroup；unavailable 的认领/下发不渲染，不置灰「已认领」',
        component: React.lazy(() => import('@/components/Disposition/DispositionBar/examples/DenseActionsDemo')),
        sourcePath: () => import('@/components/Disposition/DispositionBar/examples/DenseActionsDemo/index.tsx?raw'),
        block: true
      },
      {
        title: '7. 预警摘要 lead',
        description: '有 lead 时不渲染「处置状态」label，左侧只展示摘要（+ 可选徽章）',
        component: React.lazy(() => import('@/components/Disposition/DispositionBar/examples/LeadSummaryDemo')),
        sourcePath: () => import('@/components/Disposition/DispositionBar/examples/LeadSummaryDemo/index.tsx?raw'),
        block: true
      }
    ],
    apiDoc: [
      {
        ...
        rows: [
          { prop: 'label', desc: '左侧文案；有 lead 时不渲染', type: 'string', defaultVal: '\'处置状态\'' },
          { prop: 'lead', desc: '左侧预警摘要；有则不渲染「处置状态」label', type: 'ReactNode' },
          { prop: 'statusBadge', desc: '状态徽章（string 或 ReactNode）；可与 lead 并存', type: 'ReactNode' },
    ...
    apiDoc: [
      {
        ...
        rows: [
          ...
          { prop: 'onChange', desc: '内容变更（变量开启时含 {{key}} / msg-var-token）', type: '(html: string) => void' },
          ...
          { prop: 'variables', desc: 'catalog 变量（下拉中文+code；禁 FE DEFAULT_VARS）', type: 'VariableMentionItem[]' },
    ...
    apiDoc: [
      {
        ...
        rows: [
          { prop: 'title', desc: '分组标题', type: 'string | ReactNode' },
      ...
      },
      {
        ...
        rows: [
          { prop: 'name', desc: '列表字段名', type: 'string' },
  ...
  '/components/forminfo': {
    ...
    examples: [
      ...
      {
        title: '模态框表单示例',
        description: 'FormModal 默认壳=antd Modal + FormInfo；useFormModal 命令式；短表单无大片空白、底栏可见、提交关闭；destroyOnHidden',
    ...
    examples: [
      {
        ...
        sourcePath: () => import('@/components/Navigation/DrillNav/examples/BasicDemo/index.tsx?raw'),
        block: true
      }
    ],
    apiDoc: [
      {
        ...
        rows: [
          { prop: 'steps', desc: '步骤：id + label；已走过可传 onClick 点回', type: 'DrillStep[]', required: true },
    ...
    apiDoc: [
      {
        ...
        rows: [
          ...
          { prop: 'suffix', desc: '后缀；string/number 自动小号单位（12px）；自定义 ReactNode 原样渲染', type: 'React.ReactNode' },
          { prop: 'tone', desc: '马卡龙底色（rose/lilac/mint/blue/peach/butter）；与 style 并存时显式 style 优先', type: 'StatMacaronTone' },
  ...
  '/components/table': {
    title: 'Table 表格',
    description: '基于 antd Table 的列表包装：默认横向滚动、受控分页、空态 Empty。支持单/多/三级表头、列配置与行隐藏；Fetch 模式（fetchData / fetchUrl）由 Table 接管 pageData 分页。',
    examples: [
      {
        title: '单表头',
      ...
      {
        title: 'Fetch 拉数',
        description: 'fetchData（Marsun pageData）+ fetchUrl（Form 同构）；showcase 假数据非业务 mock；筛变回第 1 页、ref.reload',
    ...
    apiDoc: [
      {
        ...
        rows: [
          { prop: 'pagination', desc: 'false 关闭；对象时合并默认 showSizeChanger、showTotal「共 n 项」；Fetch 模式下 current/pageSize/total/onChange 由 Table 接管', type: 'false | TablePaginationConfig', defaultVal: '合并默认' },
          { prop: 'scroll', desc: '滚动配置；未传时默认 { x: \'max-content\' }', type: 'TableProps[\'scroll\']', defaultVal: '{ x: \'max-content\' }' },
          { prop: 'locale', desc: '覆盖空态等；Fetch 失败且未传 emptyText 时用 error.message', type: 'TableProps[\'locale\']' },
          { prop: 'dataSource', desc: 'props 模式数据；传了（含 []）则忽略 fetchData/fetchUrl', type: 'RecordType[]' },
          { prop: 'fetchData', desc: 'Fetch 主路径：(query)=>Promise；优先于 fetchUrl；需 dataSource===undefined', type: '(q: TableFetchQuery) => Promise<unknown>' },
          { prop: 'fetchUrl', desc: 'Form FetchSelect 同构辅路径；配合 fetchOptions/transformData；需 dataSource===undefined', type: 'string' },
          { prop: 'fetchOptions', desc: 'RequestInit 或按 query 生成（可写入 currentPage/pageSize）', type: 'RequestInit | ((q: TableFetchQuery) => RequestInit)' },
          { prop: 'transformData', desc: 'raw → { pageData, total, extra? }；默认解析 Marsun pageData', type: '(raw: unknown) => TableFetchMapped<T>' },
          { prop: 'fetchParams', desc: '筛选项；稳定序列化变则回第 1 页再请求', type: 'unknown' },
          { prop: 'enabled', desc: 'Tab 懒请求；false 不拉数', type: 'boolean', defaultVal: 'true' },
          { prop: 'defaultPageSize', desc: 'Fetch 模式初始 pageSize', type: 'number', defaultVal: '20' },
          { prop: 'onFetched', desc: '每次拉数结束（成功或失败）；可取 extra/mapping', type: '(r: TableFetchMapped & { error }) => void' },
          { prop: 'ref', desc: 'TableFetchHandle：{ reload() }，写后刷新', type: 'Ref<TableFetchHandle>' },
    ...
    apiDoc: [
      {
        ...
        rows: [
          { prop: 'items', desc: '标签项：key / label / color? / selected? / disabled?', type: 'TagGroupItem[]' },
          { prop: 'onItemClick', desc: '点击某一项；有回调时默认可点（cursor:pointer）', type: '(item: TagGroupItem) => void' },
          { prop: 'empty', desc: '无项占位；传 null 不渲染', type: 'React.ReactNode', defaultVal: 'null' },
          { prop: 'color', desc: '未指定 color 的项默认语义色', type: 'SemanticColor | string', defaultVal: 'SEMANTIC_COLORS.DEFAULT' },
      ...
      },
      {
        ...
        rows: [
          ...
          { prop: 'label', desc: '展示文案（可含 ×N 等）', type: 'React.ReactNode', required: true },
  ...
  '/components/tools/actiontracking': {
    title: 'ActionTracking 行动跟踪',
    description: '页壳（Shell）→ 筛选门禁 canQuery → 列表壳 → 建单/下发（SuperSelect 任务类型/分厂 + 品种分页 api）→ 跟进/取消（HTML 消毒）。EP/API 均由 App 注入；showcase 用 fixture，≠ 业务 mock。',
    examples: [
      {
        title: '1. 页壳 + 列表',
        description: 'ActionTrackingShell + FilterBar + ListTable；新建入口由 actions 注入',
        component: React.lazy(() => import('@/components/Tools/ActionTracking/examples/ShellPageDemo')),
        sourcePath: () => import('@/components/Tools/ActionTracking/examples/ShellPageDemo/index.tsx?raw'),
        block: true
      },
      {
        title: '2. 建单 / 下发',
        description: 'CreateActionModal：SuperSelect 任务类型/分厂；品种 loadVarietyPage 分页；create vs dispatch+lockedContext',
        component: React.lazy(() => import('@/components/Tools/ActionTracking/examples/CreateActionDemo')),
        sourcePath: () => import('@/components/Tools/ActionTracking/examples/CreateActionDemo/index.tsx?raw')
      },
      {
        title: '3. 筛选 canQuery',
        description: 'F1：无创建时间窗 → 提示禁扫全表；有 from+to 才可查',
        component: React.lazy(() => import('@/components/Tools/ActionTracking/examples/FilterCanQueryDemo')),
        sourcePath: () => import('@/components/Tools/ActionTracking/examples/FilterCanQueryDemo/index.tsx?raw'),
        block: true
      },
      {
        title: '4. 跟进 / 取消',
        description: 'ActionFollowModal + CancelExecutionModal；摘要 HTML 经 sanitizeActionHtml（F3）',
        component: React.lazy(() => import('@/components/Tools/ActionTracking/examples/FollowCancelDemo')),
        sourcePath: () => import('@/components/Tools/ActionTracking/examples/FollowCancelDemo/index.tsx?raw')
      },
      {
        title: '5. 权限注入',
        description: 'canCreate 开/关：App 注入 EP；core 不硬编码权限码',
        component: React.lazy(() => import('@/components/Tools/ActionTracking/examples/PermissionsDemo')),
        sourcePath: () => import('@/components/Tools/ActionTracking/examples/PermissionsDemo/index.tsx?raw'),
        block: true
      }
    ],
    apiDoc: [
      {
        componentName: 'ActionTrackingShellProps',
        rows: [
          { prop: 'title', desc: '页标题', type: 'string', defaultVal: '\'行动跟踪\'' },
          { prop: 'actions', desc: '顶栏操作（如新建）；是否传入由 App EP 决定', type: 'PageShellActionItem[]' },
          { prop: 'filterSlot', desc: '筛选条插槽', type: 'ReactNode' },
          { prop: 'listSlot', desc: '列表+分页插槽', type: 'ReactNode' },
          { prop: 'modalSlot', desc: '新建/跟进/取消等弹层', type: 'ReactNode' }
        ]
      },
      {
        componentName: 'CreateActionModalProps',
        rows: [
          { prop: 'loaders', desc: '人员级联 + 分厂选项；禁 DEMO / 禁 core 直连业务 API', type: 'CreateActionLoaders', required: true },
          { prop: 'dimensionOptions', desc: '任务类型码表（展示名取自 App 数据）', type: 'CreateActionDimensionOption[]', required: true },
          { prop: 'onSubmit', desc: '校验通过后写 API；返回可选 actionId', type: '(payload) => Promise<string | undefined>', required: true },
          { prop: 'titlePlaceholder', desc: 'F4：任务标题 placeholder，App 注入', type: 'string' },
          { prop: 'metricPlaceholder', desc: 'F4：关联指标 placeholder，App 注入', type: 'string' },
          { prop: 'lockedContext', desc: '分域只读上下文；有则不出分厂/品种/指标', type: 'LockedContextField[]' },
          { prop: 'variant', desc: '主动新建 / 预警下发', type: '\'create\' | \'dispatch\'', defaultVal: '\'create\'' }
        ]
      },
      {
        componentName: 'ActionFilterBarProps',
        rows: [
          { prop: 'list', desc: 'ReactFilter list 配置（App 注入）', type: 'unknown', required: true },
          { prop: 'canQuery', desc: 'F1：通常 = canQueryActionList；false 展示 hint', type: 'boolean', defaultVal: 'true' },
          { prop: 'queryBlockedHint', desc: '!canQuery 时提示', type: 'ReactNode' }
        ]
      },
      {
        componentName: 'ActionFollowModalProps',
        rows: [
          { prop: 'view', desc: '跟进 / 复核 / 已取消只读', type: '\'follow\' | \'review\' | \'cancelled\'', required: true },
          { prop: 'summarySlot', desc: '摘要区；可控 HTML 须 sanitizeActionHtml', type: 'ReactNode', required: true },
          { prop: 'formSlot', desc: '跟进/复核表单字段', type: 'ReactNode' },
          { prop: 'onSubmit', desc: '跟进或完成提交', type: '(values, mode) => Promise<void>' }
        ]
      }
    ]
  },
  '/components/tools/inbox': {
    ...
    examples: [
      ...
      },
      {
        ...
        component: React.lazy(() => import('@/components/Tools/Inbox/examples/InboxFixtureStoryDemo')),
        sourcePath: () => import('@/components/Tools/Inbox/examples/InboxFixtureStoryDemo/index.tsx?raw')
      }
    ],
    apiDoc: [
      {
        ...
        rows: [
          { prop: 'fetchInbox', desc: '拉取站内信列表（须按当前用户过滤）', type: '(params) => Promise<InboxBellListResult>', required: true },
          { prop: 'markRead', desc: '标记已读', type: '(id: string) => Promise<void>', required: true },
          { prop: 'onNavigate', desc: '点击条目跳转；不传则用 href 直接跳', type: '(href: string, item: InboxBellItem) => void' },
  ...
  '/components/tools/messagecenter': {
    ...
    description: '事件目录（variables SSOT）→ 模板 CRUD（StateBar actions / 插入变量色块 / 预览替换）→ dryRun/emit → InboxBell → 权限 → 空错加载。',
    examples: [
      {
        title: '1. 事件目录',
        description: 'events + catalog variables（SSOT）',
        component: React.lazy(() => import('@/components/Tools/MessageCenter/examples/EventCatalogDemo')),
        sourcePath: () => import('@/components/Tools/MessageCenter/examples/EventCatalogDemo/index.tsx?raw')
      },
      {
        title: '2. 消息模板 CRUD',
        description: 'StateBar 右 Plus 新建；「插入变量」原子色块；推送/变量真 CRUD（eventKey+templateCode）',
        component: React.lazy(() => import('@/components/Tools/MessageCenter/examples/TemplateCrudDemo')),
        sourcePath: () => import('@/components/Tools/MessageCenter/examples/TemplateCrudDemo/index.tsx?raw'),
        block: true
      },
      {
        title: '3. emit / dryRun 预览',
        description: '同一 fixture 渲染 title/summary；emit 写入 inbox',
        component: React.lazy(() => import('@/components/Tools/MessageCenter/examples/EmitDryRunDemo')),
        sourcePath: () => import('@/components/Tools/MessageCenter/examples/EmitDryRunDemo/index.tsx?raw')
      },
      {
        title: '4. InboxBell 故事',
        description: '同源条目 → 已读 → onNavigate(href)',
        component: React.lazy(() => import('@/components/Tools/MessageCenter/examples/InboxStoryDemo')),
        sourcePath: () => import('@/components/Tools/MessageCenter/examples/InboxStoryDemo/index.tsx?raw')
      },
      {
        title: '5. 权限对照',
        description: 'canWrite 开/关：只读 vs 可写（不硬编码业务权限码）',
        component: React.lazy(() => import('@/components/Tools/MessageCenter/examples/PermissionsDemo')),
        sourcePath: () => import('@/components/Tools/MessageCenter/examples/PermissionsDemo/index.tsx?raw'),
        block: true
      },
      {
        title: '6. 空 / 错 / 加载',
        description: 'core Empty / Alert、PageSpin 加载态',
        component: React.lazy(() => import('@/components/Tools/MessageCenter/examples/EmptyErrorLoadingDemo')),
        sourcePath: () => import('@/components/Tools/MessageCenter/examples/EmptyErrorLoadingDemo/index.tsx?raw'),
        block: true
      },
      {
        title: '7. App 切换（Phase B）',
        description: 'MsgCenterAppSwitcher：仅有权+可切换对；展示名取 SSO name；单权不渲染',
        component: React.lazy(() => import('@/components/Tools/MessageCenter/examples/AppSwitchDemo')),
        sourcePath: () => import('@/components/Tools/MessageCenter/examples/AppSwitchDemo/index.tsx?raw')
      }
    ],
    apiDoc: [
      {
        ...
        rows: [
          { prop: 'fetchTemplates', desc: '拉取模板列表', type: '() => Promise<MessageTemplateAdminItem[]>', required: true },
          { prop: 'saveTemplate', desc: '保存模板', type: '(item) => Promise<void>' },
          { prop: 'fetchEventCatalog', desc: '事件目录；须回 payload（events+variables/contextSchema），禁只回数组', type: '() => Promise<MessageEventCatalogItem[] | MessageEventCatalogPayload>' },
          { prop: 'canWrite', desc: '业务注入写权限；core 不硬编码权限码', type: 'boolean' },
          { prop: 'renderAudienceField', desc: '受众树插槽', type: '(ctx) => ReactNode' },
          { prop: 'pushRulesSlot', desc: '推送规则 Tab；优先 fetchPushRules DI；未接线时可 pushRulesSlot', type: 'ReactNode' },
          { prop: 'templateVariables', desc: '已废弃：仅 fixture；业务须走 catalog variables', type: 'MessageTemplateVariable[]' }
        ]
      }
    ]
  },
  ...
  '/components/tour': {
    ...
    apiDoc: [
      ...
      },
      {
        ...
        rows: [
          { prop: 'selector', desc: 'CSS 选择器；缺失时返回 null（勿回落 body）', type: 'string', required: true }
        ]
      },
      {
        componentName: 'getTargetByTourId',
        rows: [
          { prop: 'tourId', desc: '对应 [data-tour=tourId]', type: 'string', required: true }
        ]
      }
    ]
  },
