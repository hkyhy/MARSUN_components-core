...
export const EXAMPLE_REGISTRY: Record<string, ExampleGroup> = {
  ...
  '/components/infopage': {
    ...
    examples: [
      ...
      {
        title: '平铺指标 stack',
        description: 'Content layout=stack：FlexBox 平铺，够宽一行排完；分区用 InfoPage.Part',
        component: React.lazy(
          () => import('@/components/InfoPage/examples/InfoPageContentStackDemo'),
        ),
        sourcePath: () =>
          import('@/components/InfoPage/examples/InfoPageContentStackDemo/index.tsx?raw'),
        block: true,
      },
      {
        title: '描述列表',
  ...
  '/components/tools/actiontracking': {
    ...
    description:
      '页壳（Shell）→ 筛选门禁 canQuery → 列表壳 → 建单/下发（SuperSelect 任务类型/分厂 + 品种分页 api）→ 跟进/取消（HTML 消毒）。EP/API 均由 App 注入；showcase 用 fixture，≠ 业务 mock。',
    examples: [
      {
        title: '1. 页壳 + 列表',
        description: 'ActionTrackingShell + FilterBar + ListTable；新建入口由 actions 注入',
        component: React.lazy(
          () => import('@/components/Tools/ActionTracking/examples/ShellPageDemo'),
        ),
        sourcePath: () =>
          import('@/components/Tools/ActionTracking/examples/ShellPageDemo/index.tsx?raw'),
        block: true,
      },
      {
        title: '2. 建单 / 下发',
        ...
        sourcePath: () =>
          import('@/components/Tools/ActionTracking/examples/CreateActionDemo/index.tsx?raw'),
      },
      {
        title: '3. 筛选 canQuery',
        description: 'F1：无创建时间窗 → 提示禁扫全表；有 from+to 才可查',
        component: React.lazy(
          () => import('@/components/Tools/ActionTracking/examples/FilterCanQueryDemo'),
        ),
        sourcePath: () =>
          import('@/components/Tools/ActionTracking/examples/FilterCanQueryDemo/index.tsx?raw'),
        block: true,
      },
      {
        title: '4. 跟进 / 取消',
        description:
          'ActionFollowModal + CancelExecutionModal；摘要 HTML 经 sanitizeActionHtml（F3）',
        component: React.lazy(
          () => import('@/components/Tools/ActionTracking/examples/FollowCancelDemo'),
        ),
        sourcePath: () =>
          import('@/components/Tools/ActionTracking/examples/FollowCancelDemo/index.tsx?raw'),
      },
      {
        title: '5. 权限注入',
        description: 'canCreate 开/关：App 注入 EP；core 不硬编码权限码',
        component: React.lazy(
          () => import('@/components/Tools/ActionTracking/examples/PermissionsDemo'),
        ),
        sourcePath: () =>
          import('@/components/Tools/ActionTracking/examples/PermissionsDemo/index.tsx?raw'),
        block: true,
      },
    ],
    apiDoc: [
      {
        ...
        rows: [
          ...
          {
            prop: 'actions',
            desc: '顶栏操作（如新建）；是否传入由 App EP 决定',
            type: 'PageShellActionItem[]',
          },
      ...
      },
      {
        ...
        rows: [
          {
            prop: 'loaders',
            desc: '人员级联 + 分厂选项；禁 DEMO / 禁 core 直连业务 API',
            type: 'CreateActionLoaders',
            required: true,
          },
          {
            prop: 'dimensionOptions',
            desc: '任务类型码表（展示名取自 App 数据）',
            type: 'CreateActionDimensionOption[]',
            required: true,
          },
          {
            prop: 'onSubmit',
            desc: '校验通过后写 API；返回可选 actionId',
            type: '(payload) => Promise<string | undefined>',
            required: true,
          },
      ...
      },
      {
        ...
        rows: [
          {
            prop: 'list',
            desc: 'ReactFilter list 配置（App 注入）',
            type: 'unknown',
            required: true,
          },
          {
            prop: 'canQuery',
            desc: 'F1：通常 = canQueryActionList；false 展示 hint',
            type: 'boolean',
            defaultVal: 'true',
          },
        ...
        rows: [
          {
            prop: 'view',
            desc: '跟进 / 复核 / 已取消只读',
            type: "'follow' | 'review' | 'cancelled'",
            required: true,
          },
          {
            prop: 'summarySlot',
            desc: '摘要区；可控 HTML 须 sanitizeActionHtml',
            type: 'ReactNode',
            required: true,
          },
