// @ts-nocheck
// Ported from https://github.com/kne-union/info-page/blob/master/src/computeColumnsValue.js
    ...
    columns.map((item) => {
      const itemValue =
        typeof item.getValueOf === 'function'
          ? item.getValueOf(dataSource, { column: item, context })
      ...
      if (
        !(item.hasOwnProperty('emptyIsPlaceholder')
          ? item.emptyIsPlaceholder
          : emptyIsPlaceholder) &&
        itemIsEmpty
      ) {
        return null;
...
export const computeColumnsDisplay = ({
  columns,
  emptyIsPlaceholder,
  valueIsEmpty,
  removeEmpty,
