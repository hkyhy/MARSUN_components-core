...
/** CKEditor Mention feed item：id 须以 marker 开头 */
...
/**
 * 将 CKEditor mention 节点规范为字面 `{key}`（可带色块 span），便于 emit/render。
 */
...
/** 读入编辑器前：token span / 纯文本 `{key}` → mention，便于续编着色 */
