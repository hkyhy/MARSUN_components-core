...
const ChatCard: React.FC<ChatCardProps> = ({ chat, datasets, onStartChat, onEdit, onDelete }) => {
  ...
  return (
    <div
      className={classNames('group', 'chat-card', styles['chat-card-chat-card'])}
    ...
    >
      <div className={classNames('chat-card-header', styles['chat-card-header'])}>
        ...
        <div className={classNames('chat-card-title-wrap', styles['chat-card-title-wrap'])}>
          <Tooltip title={chat.name}>
            <Text
              strong
              className={classNames('chat-card-title', styles['chat-card-title'])}
          ...
          <div className={classNames('chat-card-status-row', styles['chat-card-status-row'])}>
            <span
              className={classNames(
                ...
                styles['chat-card-status-dot'],
                isActive
                  ? classNames('chat-card-status-dot-active', styles['chat-card-status-dot-active'])
                  : classNames(
                      'chat-card-status-dot-inactive',
                      styles['chat-card-status-dot-inactive'],
                    ),
              )}
            />
            <Text
              className={
                isActive
                  ? classNames(
                      'chat-card-status-text-active',
                      styles['chat-card-status-text-active'],
                    )
                  : classNames(
                      'chat-card-status-text-inactive',
                      styles['chat-card-status-text-inactive'],
                    )
              }
            >
              {isActive ? '已启用' : '未启用'}
      ...
      <div className={classNames('chat-card-body', styles['chat-card-body'])}>
        <Paragraph
          type="secondary"
          className={classNames('chat-card-desc', styles['chat-card-desc'])}
          ellipsis={{ rows: 2 }}
        >
        ...
        <div className={classNames('chat-card-tags', styles['chat-card-tags'])}>
          {linkedKBs && linkedKBs.length > 0 ? (
            <div className={classNames('chat-card-tag-list', styles['chat-card-tag-list'])}>
              {linkedKBs.slice(0, 3).map((kb) => (
                <SemanticTag
                  ...
                  className={classNames('chat-card-tag', styles['chat-card-tag'])}
                >
                  {kb}
                </SemanticTag>
              ))}
            ...
            <Text
              type="secondary"
              className={classNames('chat-card-empty-kb', styles['chat-card-empty-kb'])}
            >
              未关联知识库
            </Text>
          )}
      ...
      <div
        className={classNames('chat-card-footer', styles['chat-card-footer'])}
