...
适用于 **P / S 各工程线、各仓库**（非仅 P6）。`da pm dry-run` / `sync_preview` 出现以下任一情况 → **立即停止**，不得 `--confirm-token`：

1. 任何 `CREATE module`（`origin: merged` 目标为 **0**）
2. 计划 CREATE 的 Module id 为 `M00x` 等历史轨（`my-plane` 除外）
3. 台账出现 `milestone: M*`（应改为钉表 `P*.*` / `S*.*`）
4. Module 名含 `·`（中点仅为 Issue 分隔符）

见空 `M*` / middot 壳：**禁止 CREATE**，只做 link→unlink→Archive（[plane-dingtalk-module-rules](plane-dingtalk-module-rules.md) 清壳 checklist）。
