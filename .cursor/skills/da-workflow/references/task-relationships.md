...
> 与 [task-naming](../../frontend-dev-spec/references/common/task-naming.md)、[dingtalk-hierarchy-naming](dingtalk-hierarchy-naming.md)、[pm-sync](pm-sync.md)、[plane-timeline](plane-timeline.md) 配套。  
...
仓库与 milestone 对照见 [task-naming 映射表](../../frontend-dev-spec/references/common/task-naming.md#仓库--钉钉编码映射)。
...
历史批次（`milestone: M002` 或 `S3-*` / `QA-S3-*`，**勿 rename id**）仍用上表 UUID；新 `S3.3.*` 任务沿用同一规则。

| 台账 id    | 模块域     | 与增量关系                     |
| ---------- | ---------- | ------------------------------ |
| `QA-S3-7`  | 质量预警   | S3-27/29/34～36 的前身模块任务 |
| `QA-S3-9`  | 根因分析   | S3-38 历史报告与 Rca 域        |
| `QA-S3-10` | 沙盘       | S3-38 沙盘侧历史报告按钮       |
| `QA-S3-11` | 共享 Agent | S3-37/39/40 智能体浮层         |
| `S3-37`    | 智能体 SSE | S3-38/39 的 split_from         |

## 示例：S3-37 拆分为 S3-38 / S3-39（历史）

| 项           | 说明                                                                                                      |
| ------------ | --------------------------------------------------------------------------------------------------------- |
| 背景         | S3-37「智能体 SSE 全量对接」首期交付完成后，序号七～八、十一仍待验收                                      |
| 动作         | 保留 S3-37 `已完成`；新增 S3-38（RCA 历史报告）、S3-39（会话管理）`进行中`                                |
| parent_issue | 二者均挂 `383c0dfb-…`（初步前端功能UI设计）                                                               |
| note         | `split_from: S3-37`；S3-38 `related_tasks: [QA-S3-7, QA-S3-10]`；S3-39 `related_tasks: [S3-40, QA-S3-11]` |
