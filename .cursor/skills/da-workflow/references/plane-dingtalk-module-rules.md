...
- 用 `·` 写 Module 名（`S3.3·功能开发`）——会与钉表 `S3.3-功能开发` **并成两个 Module**
- 用 `-` 写 Issue 名（`S3.3.15-预警页…`）——与任务台账显示不一致
- `da pm sync` 对 `origin: merged` milestone **CREATE** 或 **PATCH** Module 名称

历史误建（含 `·` 的空 Module）：rename `(重复·待删) …` → Plane UI Archive；任务迁到钉表 keeper 后 unlink。

## 误建 Module 清理（保留任务）

1. `plane_pull` 刷新快照
2. 保留钉表 Module（含 `dev-aanalysis:module:DT-*` 或名称 `{id}-{name}`）
3. 误建 Module（常见 `{id} · {name}` / `my-*:module:{id}`）下 Work Item：**迁移** link 到钉表 Module UUID，**不删**任务
4. 重复 WI（同名且已有层级 `external_id`）：mark `(重复·待删)` → 勿 DELETE API
5. 空 Module rename `(重复·待删) …` 后 Plane UI **Archive**
...
| 见空壳      | 活跃 `M*` / `(重复·待删) … · …` → **禁止再 CREATE**，只做迁任务 + Archive    |

### 「今日回潮」禁令（任意项目）

见空的活跃 `M*` 或 `(重复·待删) … · …` Module 时：
...
5. dry-run 验收：`CREATE module = 0`；活跃 Module 仅钉表 keeper

### 脚本硬门槛（全仓）

`sync_preview` / `sync_plane` / `validate_manifest`：若计划 **CREATE module** 且 id 匹配 `^M\d+`，或 Module 名含 `·` → **fail dry-run**，阻止 confirm-token sync。唯一例外：`my-plane` 维持 `M003-*`；**其它仓 / 项目一律禁止**。
