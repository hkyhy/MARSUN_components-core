...
- **架构师**：每次新增或更改组件，须同步更新对应规范文档与提示词（与代码同一任务内完成）；可复用踩坑见 SKILL 核心原则 #43；**拆包**：新页面默认路由 lazy；重能力进 core 时先定 L0/L1/L2，L2 走子路径（见 [bundle-tree-shaking](../common/bundle-tree-shaking-拆包与摇树.md)）。**HTTP**：业务仓只薄封装 `createMarsunRequest`，禁止平行原生 fetch 客户端与 kne Fetch 页面拉数（见 [routing-api HTTP 客户端](../business/routing-api-路由与API.md)）。
