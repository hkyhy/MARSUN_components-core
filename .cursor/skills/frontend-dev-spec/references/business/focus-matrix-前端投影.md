...
| 表头可见文案 | `leaf.metricName`；列 `title` **必须是 string**（列配置面板遇 ReactNode 会显示 key，如 `yarnOnline_…`）；禁止 sapCode 当标题；hover「名称 · code」用 `onHeaderCell` 原生 `title`，**禁止**用 `<Tooltip>` 包 `title`；**禁止**把 `display:block` 样式挂到 Column.className（会打穿 table-cell，表头竖排） |
...
| 半制品列     | `sapCode`=`列__processKey`（如 `HCL__draw1`）；AFIS 等 `sapCode==null` 显示未接入                                                                                                                                                                                                                        |
