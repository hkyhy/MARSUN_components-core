...
| `Table`                                                      | 列表 Table 包装（默认分页/滚动/Empty）。业务**必须**用此组件并传稳定 `tableName`；`columnConfigEnabled`（默认 `!!tableName`）控制是否可自定义列；开启时最右独立齿轮列（跨多级表头高度）+ fetch/save；**禁止**直连 antd `Table`（豁免：Form `TableList`、HTML table、showcase ApiDoc） |
...
| 展示/表单内容块        | `InteractiveBlock`（`@hkyhy/marsun-components-core`） | title/info/actions/subtitle/tags 层级；见 [shell-layout-页面壳与布局.md](shell-layout-页面壳与布局.md)                                                                  |
...
| 列表表格                 | `Table`（core；必填 `tableName`；`columnConfigEnabled` 控制列配置；齿轮为最右独立列；QA prefs → [platform-dev/用户偏好](../../../../backend-dev/platform-dev/用户偏好/接口.md)） | 直连 antd `Table`（禁止，存量须迁） |
