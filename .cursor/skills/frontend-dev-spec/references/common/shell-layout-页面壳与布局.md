...
**所有业务 Modal（含 FormModal / StepModal）须 `centered`**；非表单弹窗优先 core `Modal`（S/M/L 业务壳）。

**命令式 / TabsLayout / Drawer**：需要 `useModal`/`useDrawer`/`useConfirmModal`、`TabsLayout`/`ColumnsLayout`/`ScrollRegion`，或嵌套 Drawer 时，用 **`ReactModal` / `ReactDrawer`**（vendor 自 `@kne/react-modal`）。**禁止**用 ReactModal 覆盖包根 `Modal` 导出。
...
| **显性提示**（必须让用户**当下**看到并可能操作：废弃页跳转、权限不足引导、数据不可逆、强校验失败汇总、代管上下文） | core `<Alert>`（`@hkyhy/marsun-components-core`，≥ 0.1.66），`type` 按语义（`warning`/`error`/`info`/`success`），默认 `showIcon`，紧凑等宽细边框 | 仅当「不显示会丢关键上下文」时才用；文案 ≤ 2 行，含可操作按钮用 `action` / `description`。**禁止**业务直连 antd `Alert`                                                                                                                                                                                                           |

**判定顺序**：先问「用户不 hover 会不会丢关键信息？」→ 否则一律走 `Info`+`TooltipInfo`。core `<Alert type="info">` 仅留给「显性提示」场景，不得作为默认说明样式。
...
- 用 antd `<Alert type="info" banner>` 或 core `<Alert banner>` 当「页面说明条」常驻顶部（core Alert 无 banner，勿自行加满宽重 banner）
- 把可操作提示压进 tooltip（hover 才见的不能是「必须当下操作」的引导）
- 同一页面同时挂 `<Alert>` 说明 + `Info`+tooltip 说明（二选一，避免重复真相源）
- 业务直连 `import { Alert } from 'antd'`（改用 core `Alert`）
