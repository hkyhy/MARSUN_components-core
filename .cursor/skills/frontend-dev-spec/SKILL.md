...
description: |
  React 19 + antd 6 + marsun-components-core 前端规范。触发：新建/改 src/components|pages、Form/Modal/Action/Filter/列表详情、VirtualScrollbar、PageSpin、TooltipInfo、Alert/InfoIcon 提示、Permissions/IAM 齐套、core Table、Icons、样式/目录、FOCUS/module-patterns、canQuery/选择性筛选、拆包/tree shaking/React.lazy/子路径 exports/bundle、createMarsunRequest/HTTP 客户端、契约三件套、自检/循环验证。细则只读 references/（common|business|prompts）；禁止把规范全文写进 description。
---
...
11. **角色标签单一来源** `(business)`：角色显示名从 `GET /api/permissions/role-options` 获取，通过 `roleOptionsStore` / `getRoleLabel()` 使用；禁止 `ROLE_LABEL_MAP` 硬编码
    11a. **业务展示名取自数据** `(business)`：接口/库已返回的展示字段（如 IAM `SystemApp.name`、分厂 `factoryName`）直接用于分组/下拉/表格；跨页复用走既有 store/localStorage。禁止前端再造 `code → 中文` 平行表或嵌套三元兜底。细则见 [permissions-data-权限与常量.md](references/business/permissions-data-权限与常量.md)「SystemApp 展示名」、[hard-requirements 规则 2](../hard-requirements/SKILL.md)。**说明**：按 App code 选择静态 `*PermissionBindCatalog`（权限树 UI 结构）≠ SystemApp 展示名，二者勿混
...
15. **路由按模块拆分** `(business)`：页面路由放 `src/pages/{Module}/routes.tsx`，组件展示路由汇总到 `src/components/routes.tsx`。含多个子模块的业务域（如 `Common`、`AgentHub`）在 `src/components/{Domain}/routes.tsx` 单独维护嵌套子路由，`components/routes.tsx` 统一 import 引用。`App.tsx` 通过 `{xxxRoutes}` 引用各模块路由，不在 App.tsx 中内联路由定义。**页面组件须 `React.lazy`**（见 #49 · [bundle-tree-shaking](references/common/bundle-tree-shaking-拆包与摇树.md)）
...
20. **图标统一从 core 导出** `(common)`：业务项目禁止直接 `import from 'lucide-react'` 或 `@ant-design/icons`；统一从 `@hkyhy/marsun-components-core` 导入 Icons（如 `RefreshCw`、`CircleAlert`），加载态用 `spin` prop。缺图标时先在 `marsun_components-core` 的 `Icons` 模块补导出，再业务引用。**core 内 Icons 须可摇**（禁止单入口静态拉齐全部已登记 lucide）。详见 [component-mapping-组件映射.md](references/common/component-mapping-组件映射.md) Icons 节、[bundle-tree-shaking](references/common/bundle-tree-shaking-拆包与摇树.md)
21. **页面默认不放刷新按钮** `(business)`：页面/列表/详情**默认不提供「刷新」按钮**——数据加载由筛选条件变更、分页、写操作（新增/编辑/删除/审核）回调自动重载，配合 `ModulePageShell` 的 `spinning` / `usePageShellLoading` 全局 loading 即可；**禁止**为「凑操作」在 Header/工具栏默认塞 `刷新`。仅当存在**无对应触发源的缓存型重载**（如后端定时任务产出、`refreshData` 类同步接口、预览需手动重生成）时，才在该入口单独提供刷新，并使用 `refreshAction`（`Action/refreshAction.tsx`）或 listArray 的 `icon: <RefreshCw spin={loading} size={16} />`。常规 CRUD（编辑/删除/导出等）listArray **不加 icon**，只显示文字；菜单项、Timeline、Tree 等非 ButtonGroup 操作场景可保留 icon
...
26. **样式统一 SCSS Modules** `(common)`：（1）统一 SCSS，模块样式用 `style.module.scss`；（2）每个页面、每个含 JSX 的组件（含子组件、嵌套子组件、Demo）均维护 `style.module.scss`，无样式时保留空文件，目录 `{Name}/index.tsx` + `{Name}/style.module.scss`，禁止 TSX 与 scss 分离；（3）禁止 Tailwind CSS；（4）统一 `classNames(...)` 合并，禁止 `sc()` 等 helper，每个 className 含预定语义类名（kebab-case）；（5）预定 className 格式 `{组件名-kebab}-{功能定位-kebab}`，SCSS 同名，TS 用 `styles['kebab-name']`；（6）凡含 `padding`/`border` 的规则须 `box-sizing: border-box`（§8.12）。公共样式放 `src/styles/`。详见 [common/styles-样式规范.md](references/common/styles-样式规范.md)
27. **npm 导出分层** `(common)`：L0/L1 对外符号须经模块 `index.ts` → 包根 `src/index.ts`；L2 产品域（AgentHub、File 预览、Llm）经 `exports` 子路径入口（`agent-hub`/`file`/`llm`），**禁止**新 L2 只靠包根暴露且默认可被无相关 SPA 首包拖入。showcase 禁止 deep import 未导出符号。新增时同步 `component-mapping-组件映射.md`、（L2）`exports` 与 `examples/meta.json`。见 [common/bundle-tree-shaking-拆包与摇树.md](references/common/bundle-tree-shaking-拆包与摇树.md)
...
29. **Commit 关单 Plane** `(common)`：子仓库已配置 Plane 时，**每次 git commit 后**须 `timeline-sync` + `task done`（完成时）；**禁止**为关单跑 `@da pm dry-run` → 全量 sync。全量仅用户明确「整理 Plane」。新任务 id 须对齐钉钉层级（`S3.3.*` 等，见 [da-workflow/task-naming](../da-workflow/references/task-naming.md)）；**取号前 `plane_pull`，`id = max(名称 S3.3.N)+1`，勿盲信 `next_task_id`**；细分挂 **钉钉三级** `parent_issue`（非 Module），CREATE 走 `--confirm-plane`（须把父项/关系 PATCH 到 Plane，见 [task-relationships](../da-workflow/references/task-relationships.md)）；my-plane 维持 `M003-*` 例外
...
35. **非业务代码进 core** `(common)`：新建或修改 `src/utils/**` 前 **MUST** 查 `@hkyhy/marsun-components-core` 包根导出（见 [component-mapping-组件映射.md](references/common/component-mapping-组件映射.md) npm Utils 表）。**已存在于 core 的函数禁止在业务项目再写同名/同义 `src/utils/*.ts` 文件**；直接从包 import（如 `toDateTimeRange`、`recentDayRangeStrings`）。纯函数、无项目 API/store/业务枚举 → 写 core；含 zustand、业务 API、领域常量 → 留业务项目。仅允许薄配置层（`request.ts` / `api/client.ts`、`Files/download.ts` 注入 token）。**HTTP 客户端**：业务仓统一 `createMarsunRequest` 薄封装（token / 401 / SSO）；**禁止**平行原生 `fetch` 客户端（豁免：SSE `EventSource`/`fetch` 流、blob 下载旁路）。信封 `code:0`、无 `code` 的 2xx 扁平透传、blob 不读 `code`；**禁止业务侧为第三信封加 unwrap / 改 `successCode`**——见 [routing-api](references/business/routing-api-路由与API.md)「HTTP 客户端」与 [api-overview](../backend-dev-spec/references/common/api-overview-通用约定.md)。禁止业务直连 `@kne/axios-fetch` / `@kne/react-fetch` 做页面拉数。core 新增导出须同步 `src/index.ts` + component-mapping + 升版发布
...
37. **筛选项加载态与失败** `(common)`：**loading 亦须占位渲染**（禁止 `metaLoading → return null` / 整栏隐藏）；传 `loading` 给 Filter* 时 **Filter Item（Trigger）显示 `Loader2 spin`**，打开面板列表用 **Spin**（禁止 loading 时「暂无数据」）；落定空态用 core **`Empty iconType="simple"`**；失败仅 `message.error`（禁止拼接 HTTP/raw），**禁止**内联错误区；筛选优先挂 `ModulePageShell` `toolbar`（Spin 外）；**选项 loading 禁止并入内容区 `pageLoading`**；**默认选中须来自 meta/options**，**禁止**硬编码工厂 code（如 `1001`）。**S3 分厂展示名须与 `/api/v1/data-service/factories` 的 `factoryName`（dim `factory_branch`）一致**，禁止 FE 硬编码「潜山工厂」或仅靠 `shortFactoryName` 藏重复。路径级联（分厂→品种）用 **`FilterCascader`**（`leafOnly` + 可选 `onChangePath`），任意深树勾选仍用 `FilterTreeSelect`。见 [filter-筛选组件.md](references/common/filter-筛选组件.md) §5.9、[s3-factory-branch](../backend-dev-spec/references/common/s3-factory-branch-分厂branch归一.md)、[shell-layout-页面壳与布局.md](references/common/shell-layout-页面壳与布局.md)
...
39. **少 border 布局** `(common)`：模块 workarea 少 panel border；**列表项用 theme 背景块 + gap**，禁止 `border-bottom` 线分割。见 [styles-样式规范.md](references/common/styles-样式规范.md) §8.11。**带 padding/border 须 `box-sizing: border-box`**（§8.12）。
...
42. **REST 契约同任务落 backend-dev** `(common)`：新接或改造任一 REST（含「已有平台接口」如 `user_key_get`/`user_key_set`）时，**同任务**更新 marsun_arch `backend-dev/{data-dev|agent-dev|platform-dev|mock}/` 三件套（`接口.md` + OpenAPI + 测试用例）；禁止只写 `src/api/*.ts`。落点见 [backend-dev-spec / openapi-apifox §3.2](../backend-dev-spec/references/common/openapi-apifox-契约标注.md)；用户偏好示例：[platform-dev/用户偏好](../../../backend-dev/platform-dev/用户偏好/接口.md)。**接入 `userPrefs` / core Table 列配置时**：`vite.config.ts` **必须**在通配 `/api` 前增加 `/api/v1/user` → `VITE_USER_PREFS_TARGET`（或 QA `VITE_DATA_SERVICE_TARGET`）；**禁止**把该路径打到 marsun-agents 等无实现的业务 BE（见 [routing-api「用户偏好」](references/business/routing-api-路由与API.md)）
    42a. **产品/UI 字段变更同任务回写文档** `(common)`：改 Form/Modal 字段顺序、必填（`rule`）、枚举/下拉口径、去掉字段（如周期文案）时，**同任务**回写 Products PRD、Plans TODO、`backend-dev` 三件套与测试用例；若触达权限码/接线，同步权限清单与证据链。禁止只改 FE/BE 代码。
...
48. **页面/Tab 级提示：Alert vs Info+TooltipInfo** `(common)`：页面/Tab 顶部「说明性文案」**禁止默认用 `<Alert type="info" showIcon>` 当说明条**（banner 占位大、视觉重）。静态说明（视图来源/只读声明/字段口径/Tab 释义）走标题或 Tab 文字旁 `Info` icon + `TooltipInfo`（hover 展示），按内容选 `type`：`descriptions`（默认，`DescriptionItem[]` 多 label/value）/ `note`（`note={{title,description}}` 标题+描述分层，core ≥ 0.1.50）；`StateBar` 选项直接用 `info: DescriptionItem[]`（core 内部渲染 `Info`+`TooltipInfo`），`SegmentedRadio`/`Tabs` 的 `label` 为 `ReactNode` 手写内嵌 icon；仅「不显示会丢关键上下文且可能操作」（废弃跳转、权限引导、不可逆、校验失败汇总）才用 `<Alert>`（`warning`/`error`、`showIcon`、非 `banner`、紧凑）。Tab/子视图说明的 `Info` icon 放**对应选项 `label` 文字右侧**（icon 与文字同属一个按钮、归属清晰；`TooltipInfo` 走 `document.body` 不塌陷，点击 icon 选中其所属选项属预期、勿 `stopPropagation`）。见 [shell-layout §三「页面/Tab 级提示」](references/common/shell-layout-页面壳与布局.md)。
49. **拆包与摇树** `(common)`：业务页面路由默认 `React.lazy` + `Suspense`（fallback `PageSpin`）；壳/Guard 可同步。core：**L0/L1 包根 import**；**L2**（AgentHub、File 预览、Llm）走 `@hkyhy/marsun-components-core/{agent-hub|file|llm}`（迁移期根 re-export 须 `@deprecated`，新代码禁依赖）。plotly / `@ant-design/charts` / xlsx 等禁止进 App 壳。禁止 `babel-plugin-import`。细则：[common/bundle-tree-shaking-拆包与摇树.md](references/common/bundle-tree-shaking-拆包与摇树.md)。
...
| 拆包 / 摇树 / 路由 lazy / core 子路径             | [common/bundle-tree-shaking-拆包与摇树.md](references/common/bundle-tree-shaking-拆包与摇树.md) + [business/routing-api](references/business/routing-api-路由与API.md) · [component-mapping 导入约定](references/common/component-mapping-组件映射.md) |
...
| HTTP 客户端 / 信封 / 禁止平行 fetch     | [business/routing-api-路由与API.md](references/business/routing-api-路由与API.md)「HTTP 客户端」                |
...
| HTTP 客户端 / 信封               | business/routing-api「HTTP 客户端」                    | SKILL.md #35 · backend-dev-spec api-overview                                                       |
