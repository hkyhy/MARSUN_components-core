...
29. **Commit 关单 Plane** `(common)`：子仓库已配置 Plane 时，**每次 git commit 后**须 `timeline-sync` + `task done`（完成时）；**禁止**为关单跑 `@da pm dry-run` → 全量 sync。全量仅用户明确「整理 Plane」。新任务 id 须对齐钉钉层级（`S3.3.*` 等，见 [da-workflow/task-naming](../da-workflow/references/task-naming.md)）；**取号前 `plane_pull`，`id = max(名称 S3.3.N)+1`，勿盲信 `next_task_id`**；细粒度挂钉表 V0.2 大颗粒 `parent_issue`（见 [task-relationships](../da-workflow/references/task-relationships.md)）；my-plane 维持 `M003-*` 例外
